...
**What was built:**
- `contestStore.ts`: `getMainSlSquad`, `previewLeagueByCode`, `createPrivateLeague`, `joinLeagueByCode` — same shape and behavior as `db.js`'s versions (fixed 3-member cap, friendly duplicate-name error, blocking non-fatal M1 backfill via `backfill_shared_squad_history`).
- `PrivateLeagueModal.tsx` (new): Create/Join tabs, opened from a new "🔒 Create or join a private league" row at the bottom of `ContestPicker`'s list (My XI tab). Same team-name simplification as the web fix from earlier this session: creating never asks for a team name (always mirrors your SL squad's own name); joining only asks for one if the code turns out to belong to a custom-rules league.
...
- `RulesScreen.tsx`: both private-league copy blocks rewritten — no more "Go to Home → Season Long → Leagues" (web-only navigation that doesn't exist on mobile), now points at the actual mobile flow, and the concept cards describe standard-vs-custom correctly instead of implying every league has admin-configurable rules/boosters.

Deliberately not built as part of this phase: a "Your Leagues" browsing screen with switch/shared-XI-pill affordances matching web's Leagues tab layout — mobile's existing `ContestPicker` already serves as "pick which contest to work on," and extending it to look exactly like web's list is a polish item, not required for create/join to work correctly.

_Exit check: on a test account signed into the mobile app, create a standard-rules private league from My XI → 🔒 Create or join a private league, confirm the invite code works to join from a second test account, and confirm the second account's league history/boosted scores from already-played matches show up immediately (Phase 2's backfill) with no extra steps — this should require no mobile-side scoring logic at all, since it inherits Phase 1–3's fixes through the same backend calls and the same DB trigger. Also confirm a signed-in account that is NOT a member of a given private league no longer sees it at all in `ContestPicker` (migration_v53)._
