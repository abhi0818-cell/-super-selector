...
- **Squad name — resolved, shipped alongside Phase 3.** The create/join forms both asked for "Your team name" even for a standard (shared) league, where the squad IS your main SL squad — same "just a copy in another folder" logic as §3, extended to the display name, not just the picks. Agreed: for a standard league there's nothing to separately name, so it's no longer asked for — `index.html`'s create form (always standard, since it never sets custom rules/boosters) now silently uses your main SL squad's own name; the join form only asks when the invite code turns out to belong to a genuinely independent (custom-rules) league, since that squad picks separately and a distinct name is meaningful there. Guarded: creating a standard league before you've joined the main SL league at all is now blocked with a clear message, instead of silently creating a squad with nothing to mirror a name from.
...
### Phase 4 — Mobile parity — code written, pending migration run + EAS build

Deliberately last — sequenced after Phase 0–3 so mobile inherits correct behavior instead of the current bugs (per §4d).

**Correction before building:** this section originally assumed `createPrivateLeague` / `joinLeagueByCode` / `getContestByInviteCode` were "already used by `contestStore.ts`, no new backend work." Checked directly against the actual file before writing anything — false. `contestStore.ts` only ever had a read-only `loadContests()`; none of those three functions existed anywhere in the mobile app. All three had to be written from scratch, mirroring `db.js`'s versions.

**A real, pre-existing security gap found during this investigation, fixed as part of this phase, not a mobile-only issue:** `migration_v11` added a scoped RLS policy on `contests` — private rows only readable by members. `migration_v17` (written later, for an unrelated purpose) added a *second* permissive SELECT policy, `"contests_public_read"`, `USING (true)` — it drops a policy of that same name, which didn't exist yet at that point, not v11's differently-named one. Postgres ORs permissive policies together, so ever since v17 shipped, **any signed-in user could read every contest row, including private leagues' invite codes** — verified directly in both migration files. This is what let mobile's `ContestPicker` list every private league in the tournament with no membership check, and let tapping into any of them silently create a real squad via `teamStore.ensureSquad()` with no invite-code gate at all. Fixed by `migration_v53_fix_contests_rls_leak.sql` — drops the blanket policy, replaces it with one scoped to membership + a specific admin-email carve-out (needed so the Contests admin panel keeps working). Decided not to rotate existing leagues' invite codes — small invite-only friend-group app, low risk anything was actually exploited.

**A real type gap found too:** mobile's `LeagueRuleType`/`ContestContext` had no concept of "shared XI" at all — only a rules-only `'standard' | 'custom'` axis, and `toContestContext()` hardcoded `ruleType: 'standard'` for every contest regardless of its actual `scoring_rules`/`available_boosters`. Added `isShared: boolean` to `ContestContext`, and `RealContest` now fetches and derives `hasCustomRules`/`hasCustomBoosters`/`isShared` for real, the same way `index.html`'s `isSharedXI()` does — this was decorative before, not derived from data.

**What was built:**
- `contestStore.ts`: `getMainSlSquad`, `previewLeagueByCode`, `createPrivateLeague`, `joinLeagueByCode`, `getMyLeagues` — same shape and behavior as `db.js`'s versions (fixed 3-member cap, friendly duplicate-name error, blocking non-fatal M1 backfill via `backfill_shared_squad_history`). `getMyLeagues` mirrors `db.js`'s `getUserSquads` two-query shape (fetch the caller's squads, then fetch their contests by id) rather than an embedded relational select — no precedent for that query style anywhere in this codebase, on either client.
- `PrivateLeagueModal.tsx` (new): three tabs — **Your Leagues** (every contest you have a squad in, shared-XI/custom badge, invite code, tap to switch active context — same as web's per-row Switch button), **Create**, **Join** — opened from a new "🔒 Private Leagues" row at the bottom of `ContestPicker`'s list (My XI tab). Same team-name simplification as the web fix from earlier this session: creating never asks for a team name (always mirrors your SL squad's own name); joining only asks for one if the code turns out to belong to a custom-rules league.
- `ContestPicker.tsx`: private-league subtitle now reflects real `isShared` state instead of a hardcoded "standard rules" label.
- `LeagueSelector.tsx`: added the new required `isShared` field to its `onSelect` payload (`false` — it only ever lists custom-rule leagues, per its own docstring).
- `RulesScreen.tsx`: both private-league copy blocks rewritten — no more "Go to Home → Season Long → Leagues" (web-only navigation that doesn't exist on mobile), now points at the actual mobile flow (🔒 Private Leagues), and the concept cards describe standard-vs-custom correctly instead of implying every league has admin-configurable rules/boosters.

Not touched: `HomeScreen.tsx`'s existing `NestedLeagueRow` list (private leagues nested under the SL tile) still shows a generic "Private league" subtitle rather than the shared/custom badge — same cosmetic gap `ContestPicker` had before this pass, just not fixed there too. Low priority since `PrivateLeagueModal`'s "Your Leagues" tab is now the fuller view; worth a follow-up pass if it's confusing to have two slightly different-looking lists.

_Exit check: on a test account signed into the mobile app, create a standard-rules private league from My XI → 🔒 Private Leagues → Create, confirm it shows up correctly in the Your Leagues tab (badge, invite code), confirm the invite code works to join from a second test account, and confirm the second account's league history/boosted scores from already-played matches show up immediately (Phase 2's backfill) with no extra steps — this should require no mobile-side scoring logic at all, since it inherits Phase 1–3's fixes through the same backend calls and the same DB trigger. Also confirm a signed-in account that is NOT a member of a given private league no longer sees it at all (migration_v53)._

---

_Rough sizing, single engineer already familiar with this codebase, not a commitment: Phase 0 ~1 day, Phase 1 ~2–3 days (the transfer investigation is the unknown), Phase 2 ~2 days, Phase 3 ~1 day (one migration, no app-code changes), Phase 4 ~2–3 days (larger than planned — the RLS fix and the missing backend functions were found mid-build, not in the original estimate). Phases 1 and 2 are the ones that actually fix wrong scores; 0, 3, and 4 are lower-risk and could slip without hurting correctness — except migration_v53, which is a real security fix and shouldn't slip._
